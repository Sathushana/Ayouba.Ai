...
export default function Questionnaire() {
  ...
  const mhDailyRoutine = useMemo(
    () => answers.mh_dailyRoutine || {},
    [answers.mh_dailyRoutine]
  );
  const mhRootCauses = useMemo(
    () => answers.mh_rootCauses || {},
    [answers.mh_rootCauses]
  );
  ...
  useEffect(() => {
    ...
    if (key === "substanceUseNutrition" && baseAnswer) {
      ...
      if (
        ...
        const quantityQKey =
          freqKey === "Sometimes (1–2 times a week)" ||
          freqKey === "Frequently (3–5 times a week)" ||
          freqKey === "Daily"
    ...
    if (
      ...
      if (satisfaction && conditionalFollowUps[satisfaction]) {
        if (
          typeof conditionalFollowUps[satisfaction] === "object" &&
          !Array.isArray(conditionalFollowUps[satisfaction])
        ) {
    ...
    if (
      ...
      baseAnswer &&
      baseAnswer.otherNotSure &&
      conditionalFollowUps.otherNotSure
    ) {
    ...
    if (key === "mh_dailyRoutine" && baseAnswer) {
      Object.keys(mhDailyRoutine)
        .filter((r) => mhDailyRoutine[r])
        .forEach((routineKey) => {
          const q = conditionalFollowUps[routineKey];
          if (
            routineKey === "sedentary" ||
            routineKey === "irregularMeals" ||
            routineKey === "poorSleep"
          ) {
    ...
    if (key === "mh_rootCauses" && baseAnswer) {
      Object.keys(mhRootCauses)
        .filter((c) => mhRootCauses[c])
        .forEach((causeKey) => {
  ...
  const isAnyKeySelected = useCallback(
    (multiselectAnswers) =>
      Object.entries(multiselectAnswers || {}).some(
  ...
  const isStepValid = useCallback(() => {
    ...
    if (isBranchingStep) {
      if (subStep === 0) {
        ...
        if (
          currentStepData.type === "placeholder" ||
  ...
  const handleNext = useCallback(() => {
    ...
    if (isBranchingStep && subStep === 0) {
      if (
        currentStepData.key === "exerciseLocation" &&
        answers.exerciseLocation === "Mixed / Any location"
      ) {
      ...
      const isSkippingFollowUps = (() => {
        ...
        if (currentStepData.key === "dietType") {
          return !(
            dietType === "Mostly vegetables and no meat (Vegetarian)" ||
            dietType ===
        ...
        if (
          ...
          (!substanceType ||
            substanceType.none ||
            !isAnySubstanceSelected(substanceType))
        )
          return true;
        if (
        ...
        if (
          ...
          !isAnyKeySelected(answers.substanceSituations)
        )
          return true;
        if (
        ...
        if (currentStepData.key === "exerciseLocation") {
          return (
            currentStepData.options.find(
              (opt) => opt === currentStepData.key
            ) === "Mixed / Any location"
          );
        }
        if (
          currentStepData.key === "activityTypePreference" &&
          answers.activityTypePreference?.otherNotSure !== true
        )
          return true;
        ...
        if (currentStepData.key === "mh_dailyRoutine") {
          ...
          return !(
            routineAnswers.sedentary ||
            routineAnswers.irregularMeals ||
            routineAnswers.poorSleep
          );
        }
  ...
  const handleInputChange = useCallback(
    (key, value, subKey = null, type = null, itemIndex = null) => {
      ...
      const isOtherText = key === "otherText";
      ...
      if (isOtherText) {
        ...
        setAnswers((prevAnswers) => {
          ...
          if (key === "other" && isSelected) {
            delete newSelections.otherText;
          }
  ...
  const renderRadioButtons = (options, currentAnswer, subKey = null) => {
    ...
    const mainAnswer = isAnswerObject
    ...
    const handleRadioClick = (option) => {
      ...
      if (isOtherOption) {
        ...
        handleInputChange(questionKey, option, subKey, "radio");
      }
    };
  ...
  const renderConditionalFollowUps = () => {
    ...
    const visibleFollowUpQuestions = followUpQuestions.filter((q) => {
      ...
      if (
        q.parentKey === "alcoholQuantity" ||
        q.parentKey === "wl_alcoholQuantity"
      ) {
    ...
    if (isBranchingStep) {
      if (subStep === 0) {
        ...
        if (currentStepData.type === "placeholder")
          return (
            <p className="text-gray-500 italic">
              Continue to the next step for adaptive follow-up questions.
            </p>
          );
      }
      if (subStep === 1) {
